...
namespace SmartContentAI\Utils;
...
class Logger {
    ...
    public function __construct() {
        global $wpdb;
        $this->table_name = $wpdb->prefix . 'smartcontentai_logs';
    }
    ...
    private function redact_sensitive_data( $payload ) {
        $api_keys = array(
            'smartcontentai_api_key' => get_option( 'smartcontentai_api_key', '' ),
            'smartcontentai_openrouter_api_key' => get_option( 'smartcontentai_openrouter_api_key', '' ),
            'smartcontentai_openai_api_key' => get_option( 'smartcontentai_openai_api_key', '' ),
            'smartcontentai_anthropic_api_key' => get_option( 'smartcontentai_anthropic_api_key', '' ),
        );

        $decrypted_api_keys = array();
        if ( class_exists( '\SmartContentAI\Core\Security' ) ) {
            $security = new \SmartContentAI\Core\Security();
            foreach ( $api_keys as $key => $value ) {
                if ( '' !== $value ) {
                    $decrypted = $security->decrypt_api_key( $value );
                    if ( '' !== $decrypted ) {
                        $decrypted_api_keys[$key] = $decrypted;
                    }
                }
            }
        }

        return $this->redact_sensitive_value( $payload, $decrypted_api_keys );
    }

    private function redact_sensitive_value( $value, $api_key = null ) {
        if ( is_string( $value ) ) {
            if ( ! empty( $api_key ) ) {
