...
class SmartContentAI_Image_Generator {
    public const OPTION_API_KEY = 'smartcontentai_api_key';
    ...
    public const ERROR_IMAGE_API_ERROR      = 'smartcontentai_image_api_error';
    public const ERROR_IMAGE_DECODE_ERROR   = 'smartcontentai_image_decode_error';
    public const ERROR_IMAGE_UPLOAD_ERROR   = 'smartcontentai_image_upload_error';
    public const ERROR_WEBP_CONVERT_ERROR   = 'smartcontentai_webp_conversion_error';
    ...
    private ?SmartContentAI_API_Client $text_client;
    ...
    public function __construct( ?SmartContentAI_API_Client $text_client = null, array $args = array() ) {
        ...
        $this->image_model = (string) ( $args['image_model'] ?? apply_filters( 'smartcontentai_gemini_image_model', 'imagen-3.0-generate-001' ) );

        $this->max_retries             = (int) ( $args['max_retries'] ?? 2 );
        $this->initial_backoff_seconds = (float) ( $args['initial_backoff_seconds'] ?? 1.0 );

        $this->rate_limit_per_minute = (int) ( $args['rate_limit_per_minute'] ?? apply_filters( 'smartcontentai_rate_limit_per_minute', 60 ) );
    ...
    public function generate_image_base64( string $image_prompt, array $args = array() ) {
        ...
        $image_prompt = apply_filters( 'smartcontentai_image_prompt', $image_prompt, array( 'type' => $type ) );
    ...
    public function generate_and_upload( string $image_prompt, string $context_title = '', array $args = array() ) {
        ...
        $filename_base = sanitize_title( $context_title ?: 'smartcontentai-image' );
        $filename      = $filename_base ? $filename_base . '-' . uniqid() . '.' . $extension : 'smartcontentai-' . uniqid() . '.' . $extension;

        $tmp_file = function_exists( 'wp_tempnam' ) ? wp_tempnam( $filename ) : tempnam( sys_get_temp_dir(), 'smartcontentai_' );
        if ( ! $tmp_file ) {
    ...
    private function generate_alt_text( string $image_prompt, string $context_title ): string {
        ...
        if ( $this->text_client instanceof SmartContentAI_API_Client ) {
    ...
    private function wait_for_rate_limit_slot(): void {
        ...
        $history = call_user_func( $this->get_transient, SmartContentAI_API_Client::TRANSIENT_RATE_LOG );
        if ( ! is_array( $history ) ) {
            $history = array();
        }
        ...
        $history[] = $now;
        call_user_func( $this->set_transient, SmartContentAI_API_Client::TRANSIENT_RATE_LOG, $history, 60 );
    }
